...
class MapFragment : BaseFragment(), OnMapReadyCallback {
    ...
    override fun onMapReady(mapboxMap: MapboxMap) {
        ...
        mapboxMap.setStyle(currentStyle) {
            ...
            mapViewModel.boundaryCoordinates.observe(
                this,
                Observer { points ->
                    coordinates = points
    ...
    private fun addCoordinatesSource(points: ArrayList<ArrayList<Point>>, style: Style) {
        points.forEachIndexed { index, layer ->
            ...
            style.addLayerBelow(
                LineLayer("linelayer-$index", "line-source-$index").withProperties(
                    lineWidth(5f),
    ...
    private fun handleClickIcon(screenPoint: PointF): Boolean {
        ...
        if (reportFeatures != null && reportFeatures.isNotEmpty()) {
            ...
            features.forEachIndexed { index, feature ->
                if (selectedFeature.getProperty(PROPERTY_MARKER_REPORT_ID) == feature.getProperty(
                        PROPERTY_MARKER_REPORT_ID
                    )
                ) {
        ...
        if (checkInFeatures != null && checkInFeatures.isNotEmpty()) {
            ...
            features.forEachIndexed { index, feature ->
                if (selectedFeature.getProperty(PROPERTY_MARKER_CHECKIN_ID) == feature.getProperty(
                        PROPERTY_MARKER_CHECKIN_ID
                    )
                ) {
                    features[index]?.let { setFeatureSelectState(it, true) }
                } else {
    ...
    private fun setupLineLayer(it: Style) {
        it.addLayer(
            LineLayer(LINE_CHECK_IN_ID, SOURCE_CHECK_IN).apply {
                withProperties(
    ...
    private fun setupWindowInfo(it: Style) {
        it.addLayer(
            SymbolLayer(WINDOW_INFO_CHECK_IN_ID, SOURCE_CHECK_IN).apply {
                withProperties(
    ...
    private fun observeData() {
        // observe reports
        mapViewModel.getReports().observe(
            this,
            Observer { reports ->
...
// 		})
        ...
        // observe check-ins
        mapViewModel.getCheckIns().observe(
            this,
            Observer { checkins ->
            ...
            override fun onList(offlineRegions: Array<out OfflineRegion>?) {
                if (offlineRegions?.size != null) {
                    if (offlineRegions.isNotEmpty()) {
                        val bounds =
                            (offlineRegions[offlineRegions.size - 1].definition as OfflineTilePyramidRegionDefinition).bounds
                        val regionZoom =
                            (offlineRegions[offlineRegions.size - 1].definition as OfflineTilePyramidRegionDefinition).minZoom
