...
class LocationDb(val realm: Realm) {
    ...
    fun unsent(): List<CheckIn> {
        return realm.copyFromRealm(
            realm.where(CheckIn::class.java).equalTo("synced", false).findAll().createSnapshot().toList()
        )
    }
    ...
    fun allForDisplay(): RealmResults<CheckIn> {
        return realm.where(CheckIn::class.java).greaterThan(
            "timestamp",
            System.currentTimeMillis() - (MAX_DISPLAY_AGE_HOURS * 3600000)
        ).findAllAsync()
    }

    fun deleteSynced(): Long {
        val query = realm.where(CheckIn::class.java).equalTo("synced", false)
            .lessThan("timestamp", System.currentTimeMillis() - (MIN_DELETION_AGE_HOURS * 3600000))
