...
class StreamDb(private val realm: Realm) {
	...
	fun getStreamsByProjectCoreId(projectCodeId: String): List<Stream> = realm.where(Stream::class.java).equalTo(Stream.STREAM_PROJECT_SERVER_ID, projectCodeId).findAll()
			?: arrayListOf()
