package org.rfcx.incidents.adapter.classifycation
        ...
        return when (viewType) {
            VIEW_CLASSIFICATION ->
                ClassificationHolder(
                    inflater.inflate(R.layout.item_classification, parent, false), onDetectionBoxClick
                )
            else -> ClassificationEmptyHolder(
                inflater.inflate(R.layout.item_classification_empty, parent, false),
                onDetectionBoxClick
            )
        }
    }
        ...
        fun bind(classificationBox: ClassificationEmptyBox) {
            ...
            itemView.setOnClickListener {
                ...
                onDetectionBoxClick?.invoke(
                    ClassificationBox(
                        (classificationBox.beginAt + (durationOnClickPosition * 1000)).toLong(),
                        classificationBox.endAt
                    )
                )
            }
        }
