...
class VoiceSyncWorker(private val context: Context, params: WorkerParameters)
	...
	override fun doWork(): Result {
		...
		for (voice in voices) {
			voice.responseServerId?.let { serverId ->
				val audioFileOrNull = if (voice.localPath.isNotEmpty()) createLocalFilePart("file", Uri.parse(voice.localPath), "audio/mpeg") else null
