    ...
    private fun request() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            activity.requestPermissions(
                ...
                REQUEST_PERMISSIONS_IMAGE_CAPTURE
            )
        } else {
    ...
    fun handleRequestResult(requestCode: Int, grantResults: IntArray) {
        if (requestCode == REQUEST_PERMISSIONS_IMAGE_CAPTURE) {
            if (grantResults.isEmpty() || grantResults[0] != PackageManager.PERMISSION_GRANTED ||
                ...
                val shouldProvideCamera = ActivityCompat.shouldShowRequestPermissionRationale(
                    activity,
                    Manifest.permission.CAMERA
                )
                val shouldProvideStorage = ActivityCompat.shouldShowRequestPermissionRationale(
                    activity,
