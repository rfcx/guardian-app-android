...
class PullingAlertMessageReceiver : BroadcastReceiver() {
	...
	companion object {
		...
		/**
		 * start AlarmManager
		 * @repeatTime time to repeat work in seconds
		 */
		fun startAlarmForMessageNotification(context: Context?, repeatTime: Long) {
			val alarmManager = context?.getSystemService(Context.ALARM_SERVICE) as AlarmManager?
			val intent = Intent(context, PullingAlertMessageReceiver::class.java)
			val isWorking = PendingIntent.getBroadcast(context, 0,
