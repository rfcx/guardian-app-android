...
class CameraPermissions(private val activity: Activity) {
    ...
    private fun request() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            activity.requestPermissions(arrayOf(Manifest.permission.CAMERA, Manifest.permission.WRITE_EXTERNAL_STORAGE), REQUEST_PERMISSION_IMAGE_CAPTURE)
        } else {
    ...
    fun handleRequestResult(requestCode: Int, grantResults: IntArray) {
        if (requestCode == REQUEST_PERMISSION_IMAGE_CAPTURE) {
    ...
    companion object {
        const val REQUEST_PERMISSION_IMAGE_CAPTURE = 4000
    }
}